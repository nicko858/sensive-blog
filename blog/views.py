...
def serialize_post(post):
    return {
        ...
        'slug': post.slug,
        'tags': [
            serialize_tag(tag, post.tags_count)
...
def index(request):
    most_popular_posts = Post.objects.popular().prefetch_related(
        'author',
        'tags',
        )[:COUNT_TO_SLIDER].fetch_with_comments_count().fetch_with_tags_count()

    fresh_posts = Post.objects.fresh().prefetch_related(
        'author',
        'tags',
        )[:COUNT_TO_SLIDER].fetch_with_tags_count()

    most_popular_tags = Tag.objects.popular()[:COUNT_TO_SLIDER]
    ...
    context = {
        ...
        'popular_tags': [
            serialize_tag(tag, tag.posts_count)
...
def post_detail(request, slug):
    ...
    most_popular_tags = popular_tags[:COUNT_TO_SLIDER]

    most_popular_posts = Post.objects.popular().prefetch_related(
        'author',
        'tags',
        )[:COUNT_TO_SLIDER].fetch_with_comments_count()\
